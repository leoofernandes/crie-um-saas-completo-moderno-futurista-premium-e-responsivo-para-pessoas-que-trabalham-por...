...
function SettingsPage() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (!user) { setStatus("Sessão expirada."); setBusy(false); return; }
    const { error } = await supabase.from("profiles").update({ full_name: name, whatsapp }).eq("id", user.id);
    setStatus(error ? error.message : "Perfil salvo com sucesso.");
    setBusy(false);
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
      ...
      <form onSubmit={save} className="mt-6 space-y-5 rounded-md border border-border bg-card p-5">
        <label className="block text-sm">Nome completo
          <input className="mt-2 w-full rounded-md border border-input bg-background p-2" value={name || p.data?.full_name || ""} onChange={(e) => setName(e.target.value)} />
        </label>
        <label className="block text-sm">WhatsApp
          <input className="mt-2 w-full rounded-md border border-input bg-background p-2" value={whatsapp || p.data?.whatsapp || ""} onChange={(e) => setWhatsapp(e.target.value)} />
        </label>
