...
import {
  Bell, Car, Contact, FileChartColumn, Globe2, LayoutDashboard,
...
function NavLinks({ onSelect }: { onSelect?: () => void }) {
  ...
  return <nav className="space-y-5 px-3 py-5">{groups.map((group) => <div key={group.label}>
    <p className="mb-2 px-3 text-[10px] font-semibold uppercase text-muted-foreground/55">{group.label}</p>
    <div className="space-y-1">{group.items.map(({ to, label, icon: Icon }) => {
      ...
      return <Link key={to} to={to} onClick={onSelect} className={cn(
        ...
        active ? "bg-sidebar-accent text-sidebar-foreground" : "text-sidebar-foreground/55 hover:bg-sidebar-accent/55 hover:text-sidebar-foreground",
      )}>
        {active && <span className="absolute inset-y-2 left-0 w-0.5 rounded-full bg-brand" />}
        <Icon className={cn("size-[17px] transition-colors", active ? "text-brand" : "text-sidebar-foreground/45 group-hover:text-sidebar-foreground")} strokeWidth={1.7} />
        <span>{label}</span>
      </Link>;
    })}</div>
  </div>)}</nav>;
...
function UserBadge() {
  ...
  const name = profile.data?.full_name?.trim() || profile.data?.email?.split("@")[0] || "Operador";
  return <div className="hidden items-center gap-3 border-l border-border pl-4 sm:flex">
    <div className="grid size-9 place-items-center rounded-lg bg-brand/12 text-xs font-semibold text-brand">{name.slice(0, 2).toUpperCase()}</div>
    <div className="hidden leading-tight xl:block"><p className="max-w-32 truncate text-xs font-medium text-foreground">{name}</p><p className="text-[10px] text-muted-foreground">Gestor da frota</p></div>
  </div>;
}
...
export function AppShell() {
  ...
  return <div className="min-h-screen bg-obsidian font-dashboard lg:grid lg:grid-cols-[248px_1fr]">
    ...
    <main className="min-w-0 pb-20 lg:pb-0">
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border/70 bg-obsidian/85 px-4 backdrop-blur-xl sm:px-6 lg:h-20 lg:px-8">
        <div className="flex items-center gap-4"><div className="lg:hidden"><Logo /></div><div className="hidden lg:block"><p className="text-sm font-medium">Centro de controle</p><p className="text-xs text-muted-foreground">Sua operação em tempo real</p></div></div>
        <div className="flex items-center gap-1 sm:gap-3"><Button asChild size="icon" variant="ghost" className="rounded-lg" aria-label="Notificações"><Link to="/app/notificacoes"><Bell className="size-[18px]" /></Link></Button><UserBadge />
          <Sheet><SheetTrigger asChild><Button size="icon" variant="ghost" className="rounded-lg lg:hidden" aria-label="Abrir menu"><Menu /></Button></SheetTrigger><SheetContent side="right" className="w-72 bg-sidebar p-0"><SheetHeader className="border-b border-sidebar-border p-5"><SheetTitle><Logo /></SheetTitle></SheetHeader><NavLinks /></SheetContent></Sheet>
        </div>
      </header>
      <Outlet />
    </main>
