...
function CatalogPage() {
  ...
  const save = async () => {
    ...
    if (!user) { setStatus("Sessão expirada."); return; }
    const payload = { is_published: current, display_name: site.data?.display_name ?? "Meu catálogo", slug: site.data?.slug ?? user.id };
    const result = site.data
      ? await supabase.from("public_sites").update({ is_published: current }).eq("id", site.data.id)
      : await supabase.from("public_sites").insert({ ...payload, user_id: user.id });
    setStatus(result.error ? result.error.message : "Catálogo salvo com sucesso.");
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <div className="mt-6 rounded-md border border-border bg-card p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{current ? "Seu catálogo está publicado." : "Seu catálogo está oculto."}</p>
        <button onClick={() => setPublished(!current)} className="mt-4 rounded-md border border-border px-4 py-2">{current ? "Despublicar" : "Publicar catálogo"}</button>
        <button onClick={save} className="ml-3 rounded-md bg-primary px-4 py-2 text-primary-foreground">Salvar</button>
        {current && site.data && <p className="mt-4 text-sm text-brand">Link: /catalogo/{site.data.slug}</p>}
